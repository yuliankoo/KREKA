...
/* ================= HOME ================= */
const PANES=[
 {slug:'bezpecnostne-a-signalizacne-systemy',label:'Zabezpečenie',more:'Zobraziť bezpečnostné a signalizačné systémy',h:'Alarmy a signalizácia',p:'Slučkové, zbernicové, bezdrôtové aj hybridné systémy s pripojením na pult centralizovanej ochrany.',items:['Detekcia pohybu, rozbitia skla, požiaru a úniku plynov','Pripojenie na PCO Black Patrol','Požiarna signalizácia EPS','Licencia KRPZ BB č. PT 001249']},
 {slug:'kamerove-systemy',label:'Kamery',more:'Zobraziť kamerové systémy',h:'Kamerové systémy',p:'IP sieťové a analógové Turbo HD kamery pre domy, firmy, bytové domy aj obce.',items:['Nočný prísvit a čítanie ŠPZ','Vzdialený prístup zo smartfónu','Inteligentná analýza obrazu','Vlastná vysokozdvižná plošina']},
 {slug:'strukturovana-kabelaz-rozvody',label:'Siete',more:'Zobraziť štruktúrovanú kabeláž a rozvody',h:'Siete a kabeláž',p:'Optické a metalické siete LAN a slaboprúdové rozvody pri novostavbe aj rekonštrukcii.',items:['Štruktúrovaná kabeláž Cat. 5E až 7A','Aktívne a pasívne sieťové prvky','Zváranie optických vlákien','Projekcia káblových trás']},
 {slug:'registracne-pokladnice',label:'Pokladnice',more:'Zobraziť registračné pokladnice a POS systémy',h:'e-kasa pokladnice',p:'VRP, ORP a POS riešenia so servisom priamo v Brezne na Šrámkovej 2A.',items:['VRP virtuálna pokladnica','ORP online registračná pokladnica','POS riešenia pre prevádzky','Fiskalizácia a servis']},
 {slug:'vypoctova-technika',label:'IT',more:'Zobraziť výpočtovú techniku',h:'IT a výpočtová technika',p:'Počítače, servery, zálohovanie a softvér. K tomu servis na diaľku a Slovanet.',items:['Počítače, notebooky a servery','Storage a zálohovacie riešenia','Servis na diaľku cez TeamViewer','Internet, telefón a TV Slovanet']}
];
function pageHome(){
  ...
  h+='<section class="depth" id="depth" style="--n:'+PANES.length+'" aria-labelledby="prod-h"><div class="depth-stage"><header class="depth-head"><h2 id="prod-h">Čo robíme</h2><p>Päť hlavných oblastí, v ktorých navrhujeme, dodávame a servisujeme techniku.</p></header><div class="world" id="world">'+PANES.map(x=>{const pr=P(x.slug);return '<article class="pane" data-label="'+x.label+'"><div class="pane-ico"><svg viewBox="0 0 24 24">'+ICO[pr.icon]+'</svg></div><h3>'+x.h+'</h3><p>'+x.p+'</p><ul>'+x.items.map(i=>'<li>'+i+'</li>').join('')+'</ul><a class="pane-more" href="'+H('produkty/'+x.slug)+'">'+x.more+'</a></article>'}).join('')+'</div><nav class="fscale" id="fscale" aria-label="Oblasti"><small>Zaostrené na</small><i class="dot"></i></nav><div class="fdots" id="fdots" aria-hidden="true"></div></div></section>';
...
function initHome(){
  ...
  if(CAROUSEL){
    depthEl.classList.add('carousel');
    /* sekcia leží pod koncom úvodu, aby po prelete objektívu nevzniklo prázdne miesto */
    const reveal=()=>{const r=depthEl.getBoundingClientRect(),v=r.top<=1?1:clamp((heroP-.9)/.08,0,1);
      depthStage.style.opacity=v.toFixed(3);depthStage.style.visibility=v<.005?'hidden':'visible'};
    addEventListener('scroll',reveal,{passive:true});addEventListener('resize',reveal);reveal();
    fdots.innerHTML='';
    const cdots=panes.map((pane,i)=>{const b=document.createElement('button');b.type='button';b.setAttribute('aria-label',pane.dataset.label);
      b.addEventListener('click',()=>{world.scrollTo({left:pane.offsetLeft-(world.clientWidth-pane.clientWidth)/2,behavior:'smooth'})});fdots.appendChild(b);return b});
    const sync=()=>{const c=world.scrollLeft+world.clientWidth/2;let best=0,bd=1e9;
      panes.forEach((pane,i)=>{const d=Math.abs(pane.offsetLeft+pane.clientWidth/2-c);if(d<bd){bd=d;best=i}});
      cdots.forEach((b,i)=>b.classList.toggle('on',i===best))};
    world.addEventListener('scroll',()=>{clearTimeout(world._t);world._t=setTimeout(sync,60)},{passive:true});
    sync();
  }
  home={heroEl,updateHero:now=>{if(heroEl.getBoundingClientRect().bottom>0)updateHero(now);else heroVis=false},updateDepth:CAROUSEL?()=>{}:updateDepth,resize:()=>{sizeAp();measure();placeBokeh();sizeFx()},retheme:()=>{pal=null;if(fxOn)readPal()}};
...
function frame(now){
  ...
  if(sp!==lastSp){lastSp=sp;progBar.style.transform='scaleX('+sp+')';
    if(!LOWFX){blobs[0].style.transform='translate3d(0,'+(sp*40).toFixed(2)+'vh,0)';blobs[1].style.transform='translate3d(0,'+(-sp*50).toFixed(2)+'vh,0)';blobs[2].style.transform='translate3d('+(-sp*30).toFixed(2)+'vw,0,0)'}}
  tilt.x+=(tilt.tx-tilt.x)*.06;tilt.y+=(tilt.ty-tilt.y)*.06;
  if(home){home.updateHero(now);home.updateDepth()}
  if(home||!LOWFX)requestAnimationFrame(frame);else rafRunning=false;
}
let rafRunning=true;
addEventListener('scroll',()=>{if(!rafRunning){rafRunning=true;requestAnimationFrame(frame)}},{passive:true});
